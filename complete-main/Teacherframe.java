...
class Teacherframe extends JFrame implements ActionListener{
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(s.equals("借書")){
            this.setVisible(false);
            do{
                continu=LiberaryTest.liberary.getTeacher(LiberaryTest.loginnumber).BorrowBook(LiberaryTest.liberary);
                System.out.println("continue:"+continu);
            }while(continu==1);
            this.setVisible(true);
        }else if(s.equals("還書")){
            this.setVisible(false);
            LiberaryTest.liberary.getTeacher(LiberaryTest.loginnumber).returnbook(LiberaryTest.liberary.getTeacher(LiberaryTest.loginnumber));
            this.setVisible(true);
        }else if(s.equals("借閱紀錄")){
            this.setVisible(false);
            String output="";
            for(int i=0;i<LiberaryTest.liberary.getTeacher(LiberaryTest.loginnumber).BorrowBook.size();i++){
                System.out.println("我尚未借書");
                output+=LiberaryTest.liberary.getTeacher(LiberaryTest.loginnumber).BorrowBook.get(i);
                output+="\n";
            }
            if(output==""){
                output="您尚未借閱任何書籍";
            }
            new Resultframeteacher(output);
            this.dispose();
        }else if(s.equals("個人資料")){
            this.dispose();
            new Resultframeteacher(LiberaryTest.liberary.getTeacher(LiberaryTest.loginnumber).toString());
        }else if(s.equals("離開")){
            this.setVisible(false);
            this.dispose();
            ...
            LiberaryTest.liberary.getTeacher(LiberaryTest.loginnumber).setpassword(change);
            JOptionPane.showMessageDialog(null,"修改成功的密碼為"+LiberaryTest.liberary.getTeacher(LiberaryTest.loginnumber).getpassword(),"恭喜",3);
            this.setVisible(true);
        }
    }
}
